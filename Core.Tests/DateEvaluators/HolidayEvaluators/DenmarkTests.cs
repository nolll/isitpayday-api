...
public class DenmarkTests
{
    ...
    [Theory]
    [InlineData("2014-05-16")]
    [InlineData("2015-05-01")]
    [InlineData("2016-04-22")]
    public void PrayerDayIsHoliday(string date) => IsHoliday(date).Should().BeTrue();
    ...
    [Theory]
    [InlineData("2014-06-05")]
    [InlineData("2015-06-05")]
    [InlineData("2016-06-05")]
    ...
    private static bool IsHoliday(string date) => 
        HolidayEvaluator.Create(new CountryInTest("DK", "da-DK")).IsHoliday(DateTime.Parse(date));
}